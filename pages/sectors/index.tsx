  ...
  return (
    <div>
      ...
      <div className=" flex flex-col content-center items-center justify-center gap-y-10 bg-white">
        ...
        >
          <Image
            src="/static/SUB_BANNER_SECTORS.png"
            width={1264}
            height={400}
            alt="workwithus"
          />
        ...
        <div className="mx-5 flex flex-wrap items-center justify-center gap-x-10 md:gap-x-5">
          <motion.div
            className="  text-right text-5xl text-primary-20"
          ...
          <motion.div
            ...
            animate="visible"
          >
            Berbagai sistem pengkodean dan penandaan produk
            yang mampu beradaptasi dengan berbagai proses,
            jenis pengemasan, lingkungan produksi, peraturan
            dan kondisi lingkungan dari berbagai sektor.
            Keahlian internal dan keandalan sistem Zanasi
            memastikan keterlacakan total untuk produk dan
            kemasan Anda, baik untuk kemasan primer maupun
            kemasan sekunder
          </motion.div>
        </div>
        <motion.div
          className=" flex max-w-[1264px] flex-wrap justify-center  gap-5"
          variants={container}
          initial="hidden"
          animate="show"
        >
          {sectors.map((sector) => (
            <Card
              id={sector.id}
              title={sector.title}
              logoPath={sector.logo}
              key={sector.title}
              onClick={(id) => goToSector(id)}
            />
          ))}
        </motion.div>
      </div>
